...
export default ({ config, cache, pretty, mountpath }) => {
  ...
  return async (req, res, next) => {
    ...
    try {
      const [, currentService] = availableServices.includes(currentServiceName) ? await dismantle(currentServiceName, config.services[currentServiceName]) : null
      ...
      res.render('index', { mountpath, pretty, config, spec, currentService, serviceName: currentServiceName, availableServices })
    } catch (error) {
      logger.error(error)
      res.render('index', { mountpath, pretty, config, serviceName: currentServiceName, availableServices, error })
    }
  }
}
