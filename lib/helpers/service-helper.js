...
export const resolveDocument = async (url, auth = []) => {
  ...
  const headers = auth.reduce((headers, c) => Object.prototype.hasOwnProperty.call(authMethods, c.type) ? authMethods[c.type](headers, c) : headers, { })
...
export const resolveJSONDocument = async (url, auth) => {
  try {
    const response = await resolveDocument(url, auth)
...
const resolveSpecManifest = async (links = {}, auth) => {
  if (links.specs) {
    const document = await resolveJSONDocument(links.specs, auth)
    if (document._links) {
...
const resolveServiceInfo = async (links = {}, auth) => {
  if (links.info) {
    return resolveJSONDocument(links.info, auth)
  }
  return {}
}
...
export const resolve = async (hypermediaDocumentUrl, auth) => {
  ...
  const hypermediaDocument = await resolveJSONDocument(hypermediaDocumentUrl, auth)
  ...
  try {
    ...
    specs = await resolveSpecManifest(links, auth)
  } catch (e) {
    logger.warn(e)
  }
  ...
  try {
    ...
    info = await resolveServiceInfo(links, auth)
  } catch (e) {
    logger.warn(e)
  }
...
export const resolveSpec = async (spec, auth = []) => {
  const document = await resolveDocument(spec, auth)
