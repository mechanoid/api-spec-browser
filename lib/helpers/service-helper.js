...
const resolveSpecManifest = async (links = {}, auth, contextName) => {
  if (links.specs) {
    const document = await resolveDocument(links.specs, auth, contextName)
...
const resolveServiceInfo = async (links = {}, auth, contextName) => {
  if (links.info) {
    return resolveDocument(links.info, auth, contextName)
  }
  return {}
}

export const resolveSpec = async (spec, auth = []) => {
  return resolveDocument(spec, auth, 'spec')
}

export const resolve = async (hypermediaDocumentUrl, auth) => {
  if (typeof hypermediaDocumentUrl === 'object' && hypermediaDocumentUrl.spec) {
    return { standaloneSpec: await resolveSpec(hypermediaDocumentUrl.spec, auth) }
  }

  logger.info('--------- RESOLVE HYPERMEDIA DOCUMENT:', hypermediaDocumentUrl)
  const hypermediaDocument = await resolveDocument(hypermediaDocumentUrl, auth, 'root')
