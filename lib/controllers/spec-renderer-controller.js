...
const renderError = ({ res, config, configuredServices, currentServiceName }) => message => res.render('index', { config, serviceName: currentServiceName, configuredServices, error: new Error(message) })
...
export default ({ config }) => {
  ...
  return async (req, res) => {
    ...
    const error = renderError({ res, config, configuredServices, currentServiceName })
    ...
    try {
      ...
      const { specs, info, standaloneSpec } = await resolve(config.services[currentServiceName], serviceAuth, requestHost)
